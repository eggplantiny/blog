...
`reduce` 라는 함수. 처음엔 정말 어려웠다 🥲. 세상 모든 문제는 `map` 과 `filter` 두 함수면 모두 다 풀어낼 수 있을것 같았다. 하지만, 그 생각은 `reduce` 의 진가를 몰랐기 때문이다.
`reduce` 는 배열 형태로 되어있는 정보를 하나의 정보로 합칠때 굉장히 유용하다. 예를들어, 
...
function likeObjectAssign (target, source) {
  Object
...
보기 좋은 떡이 먹기도 좋다는 옛말이 처럼 코드도 딱 그런줄만 알았다. 하지만, 실무에서 이번 글과 관련된 문제를 겪고나서 드는 생각은

우리는 축약된 코드 (여기선 `Spread Operator` 를 말한다) 속 숨은 뜻을 파악 하지 않고 너무 남용하는것이 아닌가 하는 것이었다. 😌
