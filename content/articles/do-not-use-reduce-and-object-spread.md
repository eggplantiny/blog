...
# 오늘의 결론
우리 모두는 `Spread Operator` 라는 기능을 매우 유용하게 쓰고 있다. 

가령 `{ a, b, ...rest }` 같이 활용해 **나머지 매개변수**를 묶어주거나
오늘 살펴본 예제 처럼 **여러개의 오브젝트를 하나로 합쳐줄 때** 정말 유용하게 사용할 수 있다.   
하지만, 새롭게 추가된 기능이 **내부적으로 어떻게 동작하는지 이해**가 없는 상태에서 마구 남용을 하다보면 내가 개발하는 **프로덕트의 성능에 악영향**을 끼칠것은 자명하다.
